...
export default function UsuariosPanel() {
  ...
  const cargarTodosLosUsuarios = useCallback(async () => {
    try {
      ...
      console.error(
        "Error al cargar todos los usuarios para la búsqueda:",
        err
      );
  ...
  const cargarPaginaUsuarios = useCallback(
    async (page) => {
      if (page < 1) return;
  ...
  const buscarLocalmente = (texto) => {
    ...
    const filtradosCompletos = allUsuarios.filter(
      (u) =>
            ...
            <>
              ...
              {busqueda.trim() && usuariosFiltradosPagina.length === 0 && (
                <p className="no-resultados">
                  No se encontraron usuarios para la búsqueda "{busqueda}".
                </p>
              )}
            </>
          )}
