...
export default function MateriasPanel() {
    ...
    const cargarTodasLasMaterias = useCallback(async () => {
        try {
            ...
            console.error("Error al cargar todas las materias para la búsqueda:", err);
        }
    }, []);


    // Función para cargar la PÁGINA actual (paginación)
    const cargarPaginaMaterias = useCallback(async (page) => {
        if (page < 1) return;
    ...
    const buscarLocalmente = (texto) => {
        ...
        const filtradosCompletos = allMaterias.filter(m => 
    ...
    if (loading) return <h2 className="loading">Cargando materias...</h2>;
        ...
        <div className="admin-view">
            <div className="admin-card">
                ...
                <div className="lista-acordeones">
                    {/* Renderiza MateriaAcordeon */}
                    {materiasFiltradasPagina.map((materia) => (
                        <MateriaAcordeon
                            key={materia._id}
                            materia={materia} // Pasar 'materia' en lugar de 'user'
                            isOpen={openedMateria === materia._id}
                            onToggle={() =>
                                setOpenedMateria(prev => (prev === materia._id ? null : materia._id))
                            }
                        />
                    ))}
                    ...
                    {busqueda.trim() && materiasFiltradasPagina.length === 0 && (
                        <p className="no-resultados">No se encontraron materias para la búsqueda "{busqueda}".</p>
                    )}
                </div>
