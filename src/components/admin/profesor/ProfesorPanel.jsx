...
export default function ProfesoresPanel() {
  ...
  const cargarTodosLosProfesores = useCallback(async () => {
    try {
      ...
      console.error(
        "Error al cargar todos los profesores para la búsqueda:",
        err
      );
    }
  }, []);
  ...
  const cargarPaginaProfesores = useCallback(
    async (page) => {
      if (page < 1) return;
  ...
  const buscarLocalmente = (texto) => {
    ...
    const filtradosCompletos = allProfesores.filter(
      (p) =>
            ...
            <>
              {profesoresFiltradosPagina.map((profesor) => (
                <ProfesorAcordeon
                  ...
                  onToggle={() =>
                    setOpenedProfesor((prev) =>
                      prev === profesor._id ? null : profesor._id
                    )
                  }
                />
              ))}
              ...
              {busqueda.trim() && profesoresFiltradosPagina.length === 0 && (
                <p className="no-resultados">
                  No se encontraron profesores para la búsqueda "{busqueda}".
                </p>
              )}
            </>
          )}
