...
export default function CursoPanel() {
    ...
    const cargarPaginaCursos = useCallback(async (page) => {
        if (page < 1) return;
    ...
    const buscarLocalmente = (texto) => {
        ...
        const filtradosCompletos = allCursos.filter(c =>
            ...
            try {
                ...
                if (!resultados || resultados.length === 0) {
                    try {
                        ...
                        const cursosDelProfe = responseProfe.data.cursos || responseProfe.data;

                        if (cursosDelProfe && Array.isArray(cursosDelProfe) && cursosDelProfe.length > 0) {
    ...
    if (loading) return <h2 className="loading">Cargando cursos...</h2>;
                ...
                <div className="lista-acordeones">
                    {cursosFiltradosPagina.map((curso) => (
                        <CursoAcordeon
                            ...
                            onToggle={() =>
                                setOpenedCurso(prev => (prev === curso._id ? null : curso._id))
                            }
                        />
                    ))}

                    {busqueda.trim() && cursosFiltradosPagina.length === 0 && (
                        <p className="no-resultados">No se encontraron cursos para la búsqueda "{busqueda}".</p>
                    )}
                </div>
